  ...
  async listProperties(@Query('brandId') brandId: number) {
    try {
      ...
      return {
        success: true,
        properties:
          propertiesResponse.data.properties?.map((p) => ({
            name: p.name,
  ...
  async forceReconnect(@Body() body: { brandId: number }) {
    try {
      ...
      return {
        success: true,
        message:
          'Google Analytics credentials deactivated. Please reconnect your account.',
      };
    } catch (error) {
      this.logger.error(
        `Error forcing reconnection for brand ${body.brandId}:`,
        error.message,
      );
      return {
        success: false,
        message: 'Failed to process reconnection request',
      };
    }
  }
}
