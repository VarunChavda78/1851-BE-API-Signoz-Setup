  ...
  async getAnalyticsData(brandId: number, startDate: string, endDate: string) {
    try {
      ...
      if (!credentials.length || !credentials[0].propertyId) {
        return { success: false, message: 'No Google Analytics connection found or missing property ID' };
      }
...
if (new Date(credential.expiresAt) <= new Date()) {
  this.logger.log(`Token expired for credential ${credential.id}, attempting refresh`);
  
  const refreshed = await this.googleOAuthService.refreshToken(credential.id);
  if (!refreshed) {
    this.logger.error(`Token refresh failed for credential ${credential.id}`);
    return { 
      success: false, 
  ...
  this.logger.log(`Token successfully refreshed for credential ${credential.id}`);
}
      ...
      this.logger.error(`Error getting analytics data for brand ${brandId}`, error);
      return { success: false, message: 'Failed to get analytics data', error: error.message };
    }
  }
